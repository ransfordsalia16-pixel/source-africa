...
export default function SupplierMessages() {
  ...
  async function handleHide(e, orderId) {
    ...
    showToast("Conversation hidden from your inbox. It isn't deleted, and the buyer can still see it.");
    loadThreadIds();
  }

  return (
    <>
      <PageHeader title="Messages" subtitle="Chat with buyers about orders that are already underway." />
      <div className="panel-grid-2">
        ...
        <Panel title={activeOrder ? activeOrder.buyerName : "Pick a conversation"}>
          <div className="chat-list">
            {thread.map((m, i) => (
              <div key={m.id || i} className={`chat-bubble ${m.from === "supplier" ? "buyer" : "supplier"}`}>
                {m.text}
